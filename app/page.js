...
export default function Home() {
  ...
  return (
    <main className="min-h-screen flex bg-[var(--bg)] text-[var(--text)]">
      ...
      {wpLoading ? (
        ...
        <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
          {wpPosts.map((post) => (
            <a key={post.id} href={post.link} target="_blank" className="card group">
              <div className="flex justify-between items-start mb-2">
                <h3 className="font-medium text-lg group-hover:underline decoration-1 underline-offset-4">
                  {post.title.rendered}
                </h3>
      ...
      <div className="space-y-6">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {data.items.map((item, idx) => {
            const Icon = item.icon;
            return (
              <a key={idx} href={item.url} target="_blank" className="card flex items-start gap-4">
                <div className="p-2 rounded-xl bg-[var(--hover)]">
                  <Icon size={22} />
                </div>
                <div>
  ...
  return (
    <main className="min-h-screen flex bg-[var(--bg)] text-[var(--text)]">
      {/* 左侧边栏 */}
      <aside className="sidebar w-20 flex flex-col items-center py-6 border-r border-[var(--border)] sticky top-0 h-screen">
        <div className="mb-8 cursor-pointer" onClick={() => setActiveCategory('blog')} ref={logoRef}>
          <div className="w-10 h-10 rounded-xl bg-[var(--text)] text-[var(--bg)] flex items-center justify-center font-medium text-sm">L</div>
        </div>
        <nav className="flex-1 flex flex-col gap-2">
          {[
        ...
        <div className="flex flex-col gap-3">
          <a href="https://github.com/liseezn" target="_blank" className="text-[var(--text-secondary)] hover:text-[var(--text)] transition"><Github size={20} /></a>
          <a href="https://space.bilibili.com/586867478" target="_blank" className="text-[var(--text-secondary)] hover:text-[var(--text)] transition"><Video size={20} /></a>
          <a href="mailto:hi@liseezn.top" className="text-[var(--text-secondary)] hover:text-[var(--text)] transition"><Mail size={20} /></a>
        </div>
      </aside>
      ...
      {contextMenu.show && (
        <div className="context-menu" style={{ left: contextMenu.x, top: contextMenu.y }}>
          ...
          <div className="context-menu-item" onClick={() => { setContextMenu({show:false}); }}>
            复制当前时间
          </div>
        </div>
      )}
